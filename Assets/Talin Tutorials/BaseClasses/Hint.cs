 ...
 *
 *	Hints have a skillAtom property used to reference the skill with which
...
public abstract class Hint : MonoBehaviour {
	...
	void Awake () {
		if(skillAtom == null) {
			...
			if (detector == null) {
				Debug.LogWarning("No detector found for Hint, please place the hint on a detector.", transform);
			} else {
